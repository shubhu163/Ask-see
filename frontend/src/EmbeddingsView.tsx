...
export default function EmbeddingsView() {
  ...
  // Expose a manual refresh function on window for other components to call after clearing
  useEffect(() => {
    const run = async () => {
      setLoading(true)
      setError('')
    ...
    ;(window as any).__refreshEmbeddings = run
    run()
    return () => { delete (window as any).__refreshEmbeddings }
  }, [limit])
  ...
  return (
    <div style={{ border: '1px solid #ddd', padding: 12, borderRadius: 8 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
        ...
        <button onClick={() => { try { (window as any).__refreshEmbeddings?.() } catch {} }} title="Refresh" style={{ marginLeft: 8 }}>Refresh</button>
