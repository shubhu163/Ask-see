...
import './App.css'
import { ingest, ask, clearDb, uploadFile } from './api'
...
function App() {
  ...
  const onUpload = async () => {
    ...
    try {
      const data = await uploadFile(file, source, title)
  ...
  const onClear = async () => {
    if (busy) return
    setBusy(true)
    setIngestStatus('Clearing...')
    try {
      await clearDb()
      setIngestStatus('Cleared.')
      try { (window as any).__refreshEmbeddings?.() } catch {}
    } catch (e) {
      setIngestStatus('Clear failed.')
    }
    setBusy(false)
  }
    ...
    <div className="app">
      ...
      <div className="panel">
        ...
        <div className="row">
          ...
          <button onClick={onClear} disabled={busy} style={{ marginLeft: 'auto', background: '#ff5c5c' }}>Clear DB</button>
        </div>
